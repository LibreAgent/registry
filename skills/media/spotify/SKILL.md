...
Control the user's Spotify account through the LibreAgents Spotify toolset. Review the prerequisites below before connecting an account.
...
**`401 Unauthorized` after a retry** — refresh token revoked. Tell the user to run `libreagents auth spotify` again.
