...
metadata:
  libreagents:
    ...
    related_skills: [libreagents-blockchain-oracle]
---
...
- Don't use for: direct wallet transactions (use a wallet skill), on-chain analytics (use libreagents-blockchain-oracle)

## Installation

```bash
libreagents skills install smartcontractkit/chainlink-agent-skills
```

Or via direct URL:

```bash
libreagents skills install https://raw.githubusercontent.com/smartcontractkit/chainlink-agent-skills/main/SKILL.md
```
...
- [ ] Skill installed: `libreagents skills list | grep chainlink`
