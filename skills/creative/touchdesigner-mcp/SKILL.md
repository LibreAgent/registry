...
metadata:
  libreagents:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, libreagents-video]
...
```
LibreAgents -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${LIBREAGENTS_HOME:-$HOME/.libreagents}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to LibreAgents config (if missing)
...
3. **Restart LibreAgents session** to pick up the new MCP server
