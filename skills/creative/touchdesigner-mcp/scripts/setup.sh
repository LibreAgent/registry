...
LIBREAGENTS_HOME_DIR="${LIBREAGENTS_HOME:-$HOME/.libreagents}"
LIBREAGENTS_CFG="${LIBREAGENTS_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure LibreAgents config has twozero_td MCP entry ──
if [[ ! -f "$LIBREAGENTS_CFG" ]]; then
    echo -e " ${FAIL} LibreAgents config not found at ${LIBREAGENTS_CFG}"
    manual_steps+=("Create ${LIBREAGENTS_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$LIBREAGENTS_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in LibreAgents config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to LibreAgents config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$LIBREAGENTS_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${LIBREAGENTS_CFG} manually"); }
    manual_steps+=("Restart LibreAgents session to pick up config change")
fi
