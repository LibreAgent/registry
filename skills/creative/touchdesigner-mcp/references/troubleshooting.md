...
## 5. LibreAgents Configuration

### Config location

`$LIBREAGENTS_HOME/config.yaml` (defaults to `~/.libreagents/config.yaml` when `LIBREAGENTS_HOME` is unset)
...
Restart the LibreAgents session for changes to take effect. The MCP connection is
