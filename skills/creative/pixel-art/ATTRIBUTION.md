...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (LibreAgents uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — LibreAgents has `image_generate`
...
- License: MIT; see the registry's `THIRD_PARTY_LICENSE_NOTICES.md`.
- Original author of the skill shell: dodo-reach
- Expansion with palettes + video: LibreAgents contributors
