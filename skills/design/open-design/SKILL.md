...
Auto-detects 15 coding-agent CLIs from PATH (including LibreAgents) and integrates via ACP/JSON-RPC. Includes a BYOK proxy, sandboxed previews, and can import Claude Design exports.
...
The setup wizard auto-detects LibreAgents in PATH and configures the ACP/JSON-RPC integration automatically.
...
## Usage with LibreAgents

Once installed, LibreAgents auto-detects open-design via ACP/JSON-RPC. Invoke naturally:
...
2. **Video generation timeout.** Video gen (Seedance) can take 30–120 seconds. Increase LibreAgents MCP timeout if using via MCP integration.
...
4. **ACP not detecting LibreAgents.** Run `open-design detect` to re-run the CLI detection. If LibreAgents isn't in PATH, set `LIBREAGENTS_CLI_PATH` env var.
5. **BYOK proxy misconfigured.** Verify keys are exported before starting LibreAgents. The proxy reads them at startup, not per-request.

## Verification Checklist

- [ ] `npm run setup` completed without errors
- [ ] `open-design detect` shows LibreAgents in detected CLIs
