...
description: Comprehensive portfolio analysis using Alpaca MCP Server integration
  to fetch holdings and positions, then analyze asset allocation, risk metrics, individual
  stock positions, diversification, and generate rebalancing recommendations. Use
  when user requests portfolio review, position analysis, risk assessment, performance
  evaluation, or rebalancing suggestions for their brokerage account.
version: 1.0.0
author: TraderMonty
license: MIT
platforms:
...
Use Alpaca MCP Server tools to gather current portfolio information when available. In scheduled LibreAgents jobs, MCP tools may not be exposed even when Alpaca credentials are present; in that case, use the Alpaca REST API directly with `ALPACA_API_KEY`, `ALPACA_SECRET_KEY`, and `ALPACA_PAPER`.
