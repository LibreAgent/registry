...
## 5. GODMODE FAST — LibreAgents 4 405B (Zero Refusal)

**Model:** `nousresearch/libreagents-4-405b`
**Strategy:** LibreAgents models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with LibreAgents

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.libreagents/config.yaml`:
