...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("LIBREAGENTS_HOME", os.path.expanduser("~/.libreagents")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("LIBREAGENTS_HOME", Path.home() / ".libreagents")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# LibreAgents config paths
# ═══════════════════════════════════════════════════════════════════

LIBREAGENTS_HOME = Path(os.getenv("LIBREAGENTS_HOME", Path.home() / ".libreagents"))
CONFIG_PATH = LIBREAGENTS_HOME / "config.yaml"
PREFILL_PATH = LIBREAGENTS_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/LibreAgents models — already uncensored, just needs clean prompt
    "libreagents": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "libreagents" in model_lower or "nous" in model_lower:
        return "libreagents"
...
def _get_current_model() -> tuple:
    """Read current model and provider from LibreAgents config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.libreagents/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart LibreAgents for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart LibreAgents for changes to take effect.")
