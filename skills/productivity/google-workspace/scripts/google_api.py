#!/usr/bin/env python3
"""Google Workspace API CLI for LibreAgents.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing LibreAgents-facing JSON contract and falls back to the Python client
...
from _libreagents_home import get_libreagents_home
...
def _gws_binary() -> str | None:
    override = os.getenv("LIBREAGENTS_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for LibreAgents")
