#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for LibreAgents.
...
# Ensure sibling modules (_libreagents_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _libreagents_home import display_libreagents_home, get_libreagents_home

LIBREAGENTS_HOME = get_libreagents_home()
TOKEN_PATH = LIBREAGENTS_HOME / "google_token.json"
CLIENT_SECRET_PATH = LIBREAGENTS_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = LIBREAGENTS_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same LibreAgents profile to refresh consent."
    )
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to LibreAgents home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_libreagents_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for LibreAgents")
