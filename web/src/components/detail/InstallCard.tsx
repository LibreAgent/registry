...
export function InstallCard({
  ...
  // Always type-prefixed (`libreagents add skill/foo`); the CLI hits the download
  // endpoint on install, which is what increments the counter.
  const cmd = `libreagents add ${type}/${id}`;
  return (
    <div className="rounded-xl border border-default bg-elevated p-4">
      <div className="flex items-center gap-2 text-sm font-semibold text-default">
        ...
        Install with LibreAgents Client
      </div>
      ...
      <p className="mt-2.5 text-xs text-faint">
        Or browse it in the LibreAgents desktop gallery and click install.
      </p>
    </div>
  );
}
