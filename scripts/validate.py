...
ALLOWED_SKILL_LICENSES = {"MIT", "Apache-2.0"}
...
def validate_skills() -> int:
    ...
    for skill_md in iter_skill_files():
        ...
        # Recommended
        for field in ("version", "author", "platforms"):
            if not fm.get(field):
                warn(where, f"missing recommended field '{field}'")
        license_id = fm.get("license")
        if license_id not in ALLOWED_SKILL_LICENSES:
            allowed = ", ".join(sorted(ALLOWED_SKILL_LICENSES))
            err(where, f"skill license must be one of: {allowed}")
        if fm.get("version") and not SEMVER_RE.match(str(fm["version"])):
            warn(where, f"version '{fm['version']}' is not semver")

        meta = libreagents_meta(fm)
        ...
        if funding is not None:
            if not isinstance(funding, dict) or not funding.get("address"):
                err(where, "metadata.libreagents.funding must have an 'address'")
        ...
        if compat is not None:
            if not isinstance(compat, dict) or "libreagents" not in compat or "client" not in compat:
                err(where, "metadata.libreagents.compatibility must include 'libreagents' and 'client'")
        runtime_mode = meta.get("runtime_mode")
        if runtime_mode not in {"standalone", "team", "dual"}:
            err(where, "metadata.libreagents.runtime_mode must be standalone, team, or dual")
        icon_id = meta.get("icon")
        if icon_id is not None and not (
            isinstance(icon_id, str) and ICONIFY_RE.match(icon_id)
        ):
            err(where, f"metadata.libreagents.icon '{icon_id}' is not a valid Iconify id "
